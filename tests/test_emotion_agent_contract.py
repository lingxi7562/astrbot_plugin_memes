...
class EmotionAgentContractTests(unittest.TestCase):
    def test_main_keeps_private_send_tool_in_delegated_mode(self):
        ...
        self.assertIn('self.meme_agent_mode == "emotion_only"', source)
        ...
        self.assertIn("@filter.on_llm_response()", source)
        self.assertIn("reviewer.schedule(", source)
        self.assertIn("def _emotion_agent_ready", source)
        self.assertIn('"tool_use" in modalities', source)
    ...
    def test_bridge_uses_bounded_agent_tool_loop(self):
        ...
        self.assertIn("_SingleUseMemeTool.from_tool(meme_tool)", source)
        self.assertIn("ToolSet([private_meme_tool])", source)
        self.assertIn("tool_loop_agent", source)
        self.assertIn("_MAX_EXCHANGE_CHARS = 2000", source)
        self.assertIn("Message(role=\"user\", content=user_text)", source)
        self.assertIn("Message(role=\"assistant\", content=draft)", source)
        self.assertNotIn("_MAX_CONTEXT_MESSAGES", source)
        self.assertIn("asyncio.wait_for", source)
        self.assertIn("send_meme", source)
        self.assertIn("class EmotionOnlyReviewer", source)
        self.assertIn("_MAX_SEEN_AUTO_REVIEWS = 512", source)

    def test_schema_exposes_delegated_settings(self):
        schema = json.loads(Path("_conf_schema.json").read_text(encoding="utf-8"))
        self.assertEqual(
            schema["meme_agent_mode"]["options"],
            ["direct", "emotion_agent", "emotion_only"],
        )
